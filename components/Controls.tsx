...
function badgeFromScore(score: number | null) {
  if (score === null || Number.isNaN(score)) {
    return { label: '—', className: 'bg-gray-300 text-gray-800 dark:bg-gray-700 dark:text-gray-100' };
  }
  if (score >= 80) return { label: 'Green (Healthy)', className: 'bg-green-200 text-green-800 dark:bg-green-900/40 dark:text-green-300' };
  if (score >= 60) return { label: 'Yellow (OK)', className: 'bg-yellow-200 text-yellow-900 dark:bg-yellow-900/40 dark:text-yellow-300' };
  if (score >= 40) return { label: 'Orange (Caution)', className: 'bg-orange-200 text-orange-900 dark:bg-orange-900/40 dark:text-orange-300' };
  return { label: 'Red (Unhealthy)', className: 'bg-red-200 text-red-900 dark:bg-red-900/40 dark:text-red-300' };
}
...
const Controls: React.FC<Props> = ({
  ...
  const badge = badgeFromScore(healthScore);
    ...
    <div className="card p-4 space-y-5">
      ...
      <section>
        ...
        <div className="space-y-2">
          {cityScores.map((c) => {
            const cg = badgeFromScore(c.score);
            return (
              <div key={c.name} className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  {c.recommended && <Star size={14} className="text-amber-400" title="Recommended" />}
                  <span className="text-sm font-medium">{c.name}</span>
                  <span className={`text-[11px] px-2 py-1 rounded-md ${cg.className}`}>{c.score}</span>
                </div>
                <div className="flex items-center gap-2">
      ...
      <section>
        ...
        <div className={`rounded-xl px-4 py-3 ${badge.className} flex items-center justify-between`}>
